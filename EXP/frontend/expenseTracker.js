async function addExpense(event) {
    ...
    try {
        ...
        const resp = await axios.post('http://localhost:3000/expense/addExpense', {
            category,
            amount,
            email
        },
...
async function displayExpense(curr_page=1) {
    try {
       ...
       // const curPage=localStorage.getItem('curPage')
        const response = await axios.get(`http://localhost:3000/expense/fetchExpense?page=${curr_page}&perPage=${perPage}`, {
            headers: {
                'Authorization': localStorage.getItem('token')
            }
        });
...
function initiateRazorpayPayment(e) {
    ...
    axios.post('http://localhost:3000/expense/create-RazorPayId')
...
async function UpdateOrderStatus(response) {
    try {
        ...
        const resp = await axios.post('http://localhost:3000/expense/updateOrder', paymentDetails, {
            headers: {
                'Authorization': localStorage.getItem('token')
            }
        })
...
async function showLeader() {
    try {
        const response = await axios.get('http://localhost:3000/premium/fetch-leaderBoard', {
            headers: {
                'Authorization': localStorage.getItem('token')
            }
        })
...
async function downLoad(event)
{
    ...
    try 
    {
      ...
      const p1=  await axios.get('http://localhost:3000/premium/download-report',{
            headers:{
                'Authorization':localStorage.getItem('token')
            }
        })
